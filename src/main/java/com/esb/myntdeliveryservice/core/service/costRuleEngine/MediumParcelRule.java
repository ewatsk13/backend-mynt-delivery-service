...
public class MediumParcelRule implements RuleEngine{
    ...
    public boolean isValid(DeliveryDto deliveryDto) {
        ...
        log.info("Check Rule: Weight exceeds heavy parcel weight of {}kg and volume less than {}cm3", heavyParcelWeight, mediumParcelVolume);
    ...
    public BigDecimal calculateCost(DeliveryDto deliveryDto) {
        log.info("#### Execute MediumParcel for volume: {}", deliveryDto.getVolume());
        BigDecimal cost = deliveryDto.getVolume().multiply(deliveryProperties.getMediumParcelFee());
        return cost;
    }
}
